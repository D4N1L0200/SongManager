...
class GlobalSongsUI:
    ...
    def add_global_songs():
        ...
        file = st.file_uploader("Insert a MP3 or a WAV file:", type=["mp3", "wav"], key="add_song_file_uploader")
    ...
    def update_global_songs():
        ...
        if global_songs:
            ...
            song_to_update = st.selectbox("Select a song to update", [song for song in global_songs])
            if song_to_update:
                title = st.text_input("Insert the song's new title:", value=song_to_update.title)
                artist = st.text_input("Insert the song's new artist:", value=song_to_update.artist)
                genre = st.text_input("Insert the song's new genre:", value=song_to_update.genre)
                file = st.file_uploader("Insert a MP3 or a WAV file:", type=["mp3", "wav"], key=f"update_song_{song_to_update.title}_file_uploader")
                if st.button("Update song"):
                    if title != song_to_update.title or artist != song_to_update.artist or genre != song_to_update.genre or file is not None:
                        file_name = file.name if file else song_to_update.file

                        View.songs_update(song_to_update.id, song_to_update.id_library, title, artist, genre, file_name, song_to_update.count)
    ...
    def delete_global_songs():
        ...
        if global_songs:
            ...
            song_to_delete = st.selectbox("Select a song to delete", [song for song in global_songs])
            if song_to_delete:
