...
class MyPlaylistsUI:
    ...
    def main():
        ...
        with tab1:
            MyPlaylistsUI.see_my_playlists()
        with tab2:
            MyPlaylistsUI.add_music_to_playlist()
        with tab3:
            MyPlaylistsUI.create_playlist()
        with tab4:
            MyPlaylistsUI.update_playlist()
        with tab5:
            MyPlaylistsUI.delete_playlist()
    ...
    def see_my_playlists():
        ...
        if playlists:
            ...
            playlist_to_see = st.selectbox(
                "Select a playlist to see", [playlist for playlist in playlists]
            )
    ...
    def add_music_to_playlist():
        ...
        if id_liked_songs:
            ...
            if songs:
                ...
                playlist_to_add = st.selectbox(
                    "Select a playlist to add",
                    [
                        playlist
                        for playlist in View.get_owned_playlists_minus_liked(
                ...
                song_to_add = st.selectbox(
                    ...
                    [
                        song
                        for song in songs
                        if song not in View.get_songs_by_playlist(playlist_to_add.id)
                    ],
                )
                ...
                if st.button("Add song to playlist"):
                    ...
                    # if song_to_add in View.get_songs_by_playlist(playlist_to_add.id):
                    #     st.error("Song already in playlist")
                    #     return
    ...
    def create_playlist():
        ...
        if st.button("Create playlist"):
            if not name or not description:
                st.error("Please fill all the fields")
                return
            View.playlists_insert(
                0, st.session_state["user_id"], name, description, datetime.now()
            )
    ...
    def update_playlist():
        ...
        if playlists:
            ...
            playlist_to_update = st.selectbox(
                "Select a playlist to update", [playlist for playlist in playlists]
            )
            if playlist_to_update:
                name = st.text_input(
                    "Insert the playlist's new name", value=playlist_to_update.name
                )
                description = st.text_input(
                    "Insert the playlist's new description",
                    value=playlist_to_update.description,
                )

                if st.button("Update playlist"):
                    if (
                        name != playlist_to_update.name
                        or description != playlist_to_update.description
                    ):
                        View.playlists_update(
                            playlist_to_update.id,
                            playlist_to_update.id_user,
                            name,
                            description,
                            playlist_to_update.creation_date,
                        )
    ...
    def delete_playlist():
        ...
        if playlists:
            ...
            playlist_to_delete = st.selectbox(
                "Select a playlist to delete", [playlist for playlist in playlists]
            )
